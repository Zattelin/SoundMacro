...
var Roll20AM = Roll20AM || (function() {var scriptStart = new Error;
    scriptStart = scriptStart.stack.match(/apiscript\.js:(\d+)/)[1]*1;
    'use strict';

    var version = '1.11523130236',
        lastUpdate = 1523130236,
        schemaVersion = 1.11523130236,
        helpLink,
        delays = [],
        defaults = {
            css: {
    ...
    */
    outputConfig = function(menu,who){
        ...
        output = '/w "'+who+'" <div style="border: 1px solid black; background-color: white; padding: 3px 3px;">'
    ...
    importFilter = function(l,who){
        ...
        if(state.Roll20AM.playLists[l.n]){
            ...
            createList({listName:l.n,access:'gm',mode:shuffleState[l.s]},tracks,who);
        }
    },
	...
	changeVolume = function(tracks,change,who){
	    var ops = {
            '*': (a,b)=>a*b,
            '/': (a,b)=>a/b,
            '+': (a,b)=>a+b,
        ...
        nVol,
        error=false,
        adj=(''+change).match(/^\s*([+-\/*]?)(\d+(?:\.\d+)?)\s*$/);
        if(adj){
            adj[2]=parseInt(adj[2],10);
            adj[1]=adj[1]||'=';
            if(tracks==='MASTER'){
                nVol = ops[adj[1]](state.Roll20AM.masterVolume,adj[2]/100);
                if(nVol){
                    state.Roll20AM.masterVolume = nVol;
                    _.each(findObjs({type:'jukeboxtrack'}),(t)=>{
                ...
                _.each(tracks,(t)=>{
                    nVol = ops[adj[1]](state.Roll20AM.masterVolume,adj[2]);
                    if(nVol){
                        state.Roll20AM.trackDetails[t.id].volume = nVol;
                        t.set({volume:Math.min(Math.max((state.Roll20AM.trackDetails[t.id].volume * state.Roll20AM.masterVolume),0),100)});
                    }else{
                        error=true;
                    }
	...
	fadeTracks = function(tracks,target,tick,stop,volTick,who){//still occasionally passes an invalid argument for target
	//                    Array str/nmbr nmbr Str   nmbr
        ...
        if(adj){
            ...
            if(tracks==='MASTER'){
                ...
                if(_.isNumber(tickTarget)){
                    changeVolume('MASTER',tickTarget,who);
                    if(finalTarget === state.Roll20AM.masterVolume){
                        if(stop){
                            stopTrack(findObjs({type:'jukeboxtrack'}));
                        }
                    }else{
                        _.delay(fadeTracks,tick*1000,'MASTER',tickTarget,tick,stop,volTick);
                    }
                }
            }else{
                _.each(tracks,(t)=>{
                    tickTarget=undefined;
                    finalTarget = ops['*'](state.Roll20AM.masterVolume,adj[2]);
                    if(finalTarget < (t.get('volume')*state.Roll20AM.masterVolume)){
                        tickTarget = Math.max(finalTarget,state.Roll20AM.masterVolume*(t.get('volume')*1 - volTick));
                    }else if(finalTarget > (t.get('volume')*state.Roll20AM.masterVolume)){
                        tickTarget = Math.min(finalTarget,state.Roll20AM.masterVolume*(t.get('volume')*1 + volTick));
                    }
                    if(_.isNumber(tickTarget)){
                        changeVolume([t],tickTarget,who);
                        if(finalTarget === state.Roll20AM.trackDetails[t.id].volume){
                            if(stop){
                                stopTrack([t]);
                            }
                        }else{
                            _.delay(fadeTracks,tick*1000,[t],finalTarget,tick,stop,volTick,who);
                        }
                    }
                });
            }
    ...
    delayHandler = function(cmdDetails,tracks,playlists,who,restrict){
        switch(cmdDetails.action){
            ...
            case 'listImport':
                if(!restrict){
                    importList(cmdDetails.details.listName,who);
                }
                break;
            //create playlist
            ...
            case 'listCreate':
                if(!restrict){
                    if(cmdDetails.details.listName && tracks){
                        createList(cmdDetails.details,tracks,who);
                    }else{
            ...
            case 'listEdit':
                if(!restrict){
                    if(cmdDetails.details.listName){
                        editList(cmdDetails.details.listName,(cmdDetails.details.add || cmdDetails.details.remove),tracks,cmdDetails.details,who);
                    }
                }
                break;
            //Volume Control
                ...
                if(!restrict){
                    ...
                    if(tracks.length>0){
                        if(cmdDetails.details.fade){
                            if(tracks !== 'MASTER'){
                                fadeTracks(tracks,cmdDetails.details.volume,parseInt(cmdDetails.details.tickTime),cmdDetails.details.stop,parseInt(cmdDetails.details.volTick),who)
                            }else{
                                showHelp(who);
                            }
                        }else if(cmdDetails.details.volume === 'mute' || cmdDetails.details.volume === 'unmute'){
                            muteTrack(tracks,cmdDetails.details.volume,who);
                        }else{
                            changeVolume(tracks,cmdDetails.details.volume,who);
                        }
                    }else{
                        showHelp(who);
                    }
            ...
            case 'stop':
                _.each(playlists,(n)=>{
                    if(state.Roll20AM.playLists[n] && (!restrict || state.Roll20AM.playLists[n].access === 'player')){
                        playlist(n,cmdDetails.action,cmdDetails.details,who);
                    }
                });
                if(tracks){
                    stopTrack(tracks,cmdDetails.details.soft,who);
                }else{
                    showHelp(who);
                }
                break;
            case 'config':
                if(!restrict){
                    outputConfig(cmdDetails.details.menu,who);
                }
                break;
            case 'set':
                if(!restrict){
                    setPref(cmdDetails.details,who);
                }
                break;
            case 'cancelDelay':
                if(!restrict){
    ...
    HandleInput = function(msg_orig) {
        ...
        var msg = _.clone(msg_orig),
            ...
            playlists = [],
            who=idToDisplayName(msg_orig.playerid);
        //log(msg.content);
		if(msg.playerid === 'API' && state.Roll20AM.API === 'gm'){
            msg.playerid = filterObjs((o)=>{
                return(o.get('type') === 'player' && playerIsGM(o.id));
            });
            if(msg.playerid.length>0){
                msg.playerid = msg.playerid[0].id;
            }
		}
		if(playerIsGM(msg.playerid) || state.Roll20AM.restrict === 'off'){
		    restrict = false;
		}else{
		    restrict = true;
		}
	    ...
	    if(args[0] === '!roll20AM'){
            if(args[1]){
                _.each(_.rest(args,1),(cmd) =>{
                    ...
                    delays.push(setTimeout(delayHandler,cmdDetails.details.delay,cmdDetails,tracks,playlists,who,restrict));
                });
            }else{
	...
	cmdExtract = function(cmd){
        ...
        cmdSep.tracks=_.map(cmdSep.tracks,(t)=>{
            return t.trim();
        });
        cmdSep.tracks = _.reject(cmdSep.tracks,(t)=>{return t===''});
    ...
    sendError = function(err){
        ...
        sendChat('','/w gm <div style="border: 1px solid black; background-color: white; padding: 3px 3px;">'//overall div for nice formatting of control panel
            +'<div style="font-weight: bold; border-bottom: 1px solid black;font-size: 130%;">'//Control Panel Header div
            +'<b>Roll20AM Error Handling</b></div>'
            +'<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'
            +'The following error occurred:<br><pre><div style="color:red"><b>'+err.message+'<br>'+stackToSend+'</b></div></pre>Please post this error report to the <b><u>[Script forum thread](https://app.roll20.net/forum/post/4259010/script-roll20-audio-master-hear-the-dice-hear-the-action)</u></b>.'
            +'</div>'
            +'</div>');
    },
